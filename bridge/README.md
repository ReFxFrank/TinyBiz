# TinyBiz Printer Bridge 🖨️

A tiny always-on service that reads **live status from Bambu Lab printers** over their local MQTT broker and serves it as JSON, so the TinyBiz web app can auto-update whether each printer is **printing** or **idle**.
...
```
Printer(s) ──MQTT/TLS──▶ Bridge (this) ──HTTP/JSON──▶ TinyBiz in your browser
```
...
Then open <http://localhost:7070/status>. It simulates three printers cycling through printing/idle so you can wire up TinyBiz before touching real hardware. In TinyBiz, go to **Settings → Printer sync** and set the bridge URL to `http://localhost:7070`.
...
2. Install and configure:
   ...
   npm start
   ```
3. In TinyBiz: **Settings → Printer sync → Bridge URL** = `http://<bridge-host>:7070`, then on the **Manufacturing** page click **Sync live status**. Map each machine to a printer by pasting the printer's **Serial** into the machine's *Live sync ID* (Manufacturing → machine menu → Edit).
...
| `printers[].id` | Printer **serial number** — must match the machine's *Live sync ID* in TinyBiz |
